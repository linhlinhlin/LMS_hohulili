...
export interface AdminUser {
  ...
  username: string;
  email: string;
  name: string; // Maps to fullName from backend
  fullName?: string;
  role: UserRole;
  avatar?: string;
  department?: string;
  studentId?: string;
  createdAt: string; // Backend returns Instant as string
  updatedAt: string;
  isActive: boolean; // Maps to enabled from backend
  enabled?: boolean; // Keep for compatibility
  lastLogin?: string;
  loginCount?: number;
  ...
  async getAllUsers(): Promise<AdminUser[]> {
    try {
      const response = await this.http.get<{
        ...
        data: AdminUser[];
        timestamp: string
      }>(`${this.API_BASE_URL}${this.ENDPOINTS.users}/list/all`).toPromise();
      ...
      console.warn('API unavailable for getAllUsers:', error);
  ...
  async createUser(userData: { username: string, email: string, password: string, fullName?: string, role: UserRole }): Promise<AdminUser> {
    ...
    try {
      ...
      const response = await this.http.post<{
        ...
        data: AdminUser;
        timestamp: string
      }>(`${this.API_BASE_URL}${this.ENDPOINTS.users}`, userData).toPromise();
  ...
  async updateUser(userId: string, updates: Partial<AdminUser>): Promise<AdminUser> {
    ...
    try {
      const response = await this.http.put<{
        ...
        data: AdminUser;
        timestamp: string
      }>(`${this.API_BASE_URL}${this.ENDPOINTS.users}/${userId}`, updates).toPromise();
      ...
      this.handleError(error, 'Cập nhật người dùng thất bại');
      throw error;
    } finally {
      this._isLoading.set(false);
    }
  ...
  async deleteUser(userId: string): Promise<void> {
    ...
    try {
      const response = await this.http.delete<{
        ...
        message: string;
        timestamp: string
      }>(`${this.API_BASE_URL}${this.ENDPOINTS.users}/${userId}`).toPromise();
      ...
      this.handleError(error, 'Xóa người dùng thất bại');
      throw error;
    } finally {
      this._isLoading.set(false);
    }
  ...
  async toggleUserStatus(userId: string): Promise<void> {
    ...
    try {
      const response = await this.http.patch<{
        ...
        data: AdminUser;
        timestamp: string
      }>(`${this.API_BASE_URL}${this.ENDPOINTS.users}/${userId}/toggle-status`, {}).toPromise();
      ...
      if (response?.success && response.data) {
        ...
        this.errorService.showSuccess(
          `Người dùng đã được ${updatedUser.isActive ? 'kích hoạt' : 'vô hiệu hóa'} thành công!`,
          'user'
        );
      } else {
      ...
      this.handleError(error, 'Thay đổi trạng thái người dùng thất bại');
      throw error;
    } finally {
      this._isLoading.set(false);
    }
    ...
    return [
      {
        ...
        isActive: true,
        enabled: true,
        lastLogin: new Date('2024-09-22').toISOString(),
        loginCount: 156,
      ...
      },
      {
        ...
        isActive: true,
        enabled: true,
        lastLogin: new Date('2024-09-21').toISOString(),
        loginCount: 89,
        coursesCreated: 3,
      ...
      },
      {
        ...
        isActive: true,
        enabled: true,
        lastLogin: new Date('2024-09-20').toISOString(),
        loginCount: 45,
        coursesEnrolled: 2,
        totalSpent: 5000000,
