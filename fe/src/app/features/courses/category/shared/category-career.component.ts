      ...
      <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8" role="list" aria-label="Danh sách cơ hội nghề nghiệp">
          @for (card of cards(); track card.title) {
            <div class="rounded-2xl p-8 hover:shadow-lg transition-shadow duration-300"
              ...
              @if (card.salary) {
                <div class="text-lg font-semibold mb-4" [ngClass]="accentTextClass()" [attr.aria-label]="'Mức lương: ' + card.salary">{{ card.salary }}</div>
              }
              <div class="space-y-2">
                <h4 class="font-medium text-gray-900">Yêu cầu:</h4>
                <ul class="space-y-1" role="list" [attr.aria-label]="'Yêu cầu cho ' + card.title">
                  @for (req of card.requirements; track req) {
                    <li class="text-sm text-gray-600 flex items-center" role="listitem">
...
export class CategoryCareerComponent {
  ...
  containerBgClass = computed(() => 'bg-gradient-to-br from-gray-50 to-gray-100');
}
