...
/**
 * Quiz List Component - DDD Refactored
 *
 * Presentation layer component that uses application use cases
 * Follows Angular best practices and clean architecture
 */
@Component({
                ...
                <div class="p-6">
                  ...
                  <div class="flex items-start justify-between mb-4">
                    <div class="flex-1">
                      ...
                      <div class="flex items-center text-sm text-gray-500">
                        ...
                        <span class="mr-4">{{ quiz.timeLimit || 'N/A' }} phút</span>
                  ...
                  <div class="space-y-2 mb-4">
                    <div class="flex items-center text-sm text-gray-600">
                      ...
                      <span>Thời gian: {{ quiz.timeLimit || 'N/A' }} phút</span>
                    </div>
                    <div class="flex items-center text-sm text-gray-600">
                      <svg class="w-4 h-4 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
...
export class QuizListComponent implements OnInit {
  ...
  private applyFilters(): void {
    ...
    if (this.selectedStatus) {
      switch (this.selectedStatus) {
        case 'active':
          ...
          break;
        case 'completed':
          // This would need to check user's attempts - simplified for now
          break;
        case 'available':
          filtered = filtered.filter(quiz => quiz.isActive);
          break;
    ...
    if (this.selectedTimeLimit) {
      switch (this.selectedTimeLimit) {
        case 'short':
          filtered = filtered.filter(quiz => (quiz.timeLimit || 0) < 30);
          break;
        case 'medium':
          filtered = filtered.filter(quiz => {
            const time = quiz.timeLimit || 0;
            return time >= 30 && time <= 60;
          });
          break;
        case 'long':
          filtered = filtered.filter(quiz => (quiz.timeLimit || 0) > 60);
          break;
      }
    }
  ...
  startQuiz(quizId: string): void {
    const studentId = this.authService.userEmail(); // Using email as student ID for demo

    this.takeQuizUseCase.canAttemptQuiz(quizId, studentId).subscribe({
      next: (result) => {
        if (result.canAttempt) {
          ...
          this.errorService.addError({
            message: result.reason || 'Không thể tham gia quiz',
            type: 'error',
            context: 'quiz'
          });
        }
