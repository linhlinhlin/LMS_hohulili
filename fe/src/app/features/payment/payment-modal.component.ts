...
import { PAYMENT_METHODS, PaymentMethod, SepayQrData } from '../../api/client/payment.api';
...
@Component({
  ...
  template: `
    <!-- SePay QR modal shown on top when SePay selected and form submitted -->
    @if (sepayQrData()) {
      <app-sepay-qr-modal
        [qrData]="sepayQrData()!"
          ...
          @if (paymentSuccess()) {
            ...
            <div class="p-6">
              ...
              <p class="text-center text-xs text-gray-400 mt-4">
                <app-icon name="shield" size="sm" class="text-emerald-600 mr-1"/>
                @if (selectedMethod() === 'SEPAY') {
                  Thanh toán qua SePay – chuyển khoản ngân hàng an toàn.
                } @else {
...
export class PaymentModalComponent {
  ...
  // Production: VNPay + SePay. Dev: all methods
  paymentMethods = environment.production
      ? PAYMENT_METHODS.filter(m => m.code === 'VNPAY' || m.code === 'SEPAY')
      : PAYMENT_METHODS;
  ...
  async processPayment(): Promise<void> {
    ...
    try {
      if (method === 'SEPAY') {
        // SePay: show QR modal, don't close the payment modal
