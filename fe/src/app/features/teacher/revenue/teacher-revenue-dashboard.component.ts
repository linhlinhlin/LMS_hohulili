...
@Component({
      ...
      <div class="max-w-7xl mx-auto">
            ...
            <div class="flex items-center justify-between">
              <div>
                ...
                @if (payoutBalance()?.pendingBalance && payoutBalance()!.pendingBalance > 0) {
                  <p class="text-xs text-yellow-600 mt-1">{{ formatCurrency(payoutBalance()!.pendingBalance) }} đang chờ</p>
                }
              </div>
        ...
        <div class="bg-white rounded-lg shadow p-4 mb-6 flex items-center justify-between">
          <div class="flex items-center gap-4">
            ...
            <a routerLink="/teacher/revenue/bank-accounts"
              ...
              @if (bankAccounts().length > 0) {
                <span class="text-xs text-gray-400">({{ bankAccounts().length }})</span>
              }
            </a>
          </div>
        ...
        @if (!isLoading() && bankAccounts().length === 0) {
          <div class="bg-yellow-50 border border-yellow-200 rounded-xl p-4 mb-6 flex items-center gap-3">
            ...
            <p class="text-sm text-yellow-800">
              ...
              <a routerLink="/teacher/revenue/bank-accounts" class="font-semibold underline">Thêm ngay</a> để có thể yêu cầu rút tiền.
            </p>
          </div>
        }
    ...
    @if (showPayoutModal()) {
      <div class="fixed inset-0 z-50 overflow-y-auto bg-black/50" (click)="closePayoutModal()">
        <div class="flex items-center justify-center min-h-screen p-4">
          <div class="bg-white rounded-xl shadow-xl max-w-md w-full p-6" (click)="$event.stopPropagation()">
            ...
            <div class="space-y-4">
              ...
              <div>
                <label class="block text-sm font-medium text-gray-700 mb-1.5">Tài khoản nhận tiền</label>
                @if (bankAccounts().length === 0) {
                  <div class="p-3 bg-yellow-50 border border-yellow-200 rounded-lg">
                    <p class="text-sm text-yellow-800">
                      Bạn chưa có tài khoản ngân hàng.
                      <a routerLink="/teacher/revenue/bank-accounts" (click)="closePayoutModal()" class="font-semibold underline">Thêm ngay</a>
                    </p>
                  </div>
                } @else {
                  <select [(ngModel)]="selectedBankAccountId"
                    ...
                    @for (acct of bankAccounts(); track acct.id) {
                      <option [value]="acct.id">
                        {{ getBankDisplayName(acct.bankCode) }} — {{ acct.accountNumberMasked }} ({{ acct.accountName }})
                        {{ acct.isDefault ? '★ Mặc định' : '' }}
                      </option>
                    }
                  </select>
                  <a routerLink="/teacher/revenue/bank-accounts" (click)="closePayoutModal()"
                     class="mt-1 text-xs text-[#0056D2] hover:underline">
...
export class TeacherRevenueDashboardComponent implements OnInit {
    ...
    canRequestPayout(): boolean {
        ...
        return balance.availableBalance >= (balance.minPayoutAmount || 50000) &&
    ...
    openPayoutModal(): void {
        ...
        const defaultAcct = this.bankAccounts().find(a => a.isDefault) ?? this.bankAccounts()[0];
        this.selectedBankAccountId = defaultAcct?.id ?? '';
        this.payoutNote = '';
    ...
    isPayoutFormValid(): boolean {
        const balance = this.payoutBalance();
        if (!balance || !this.selectedBankAccountId) return false;
        return (
            this.payoutAmount >= (balance.minPayoutAmount || 50000) &&
    ...
    submitPayoutRequest(): void {
        ...
        this.revenueService.requestPayout({
            ...
            teacherNote: this.payoutNote || undefined,
        }).subscribe({
            next: (response) => {
            ...
            error: (e) => this.toast.error('Lỗi: ' + (e?.error?.message || 'Không thể gửi yêu cầu rút tiền'))
        });
    }
