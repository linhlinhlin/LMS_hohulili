  ...
  template: `
    <div class="min-h-screen bg-slate-50">
      ...
      <div class="bg-white border-b border-slate-200">
        <div class="max-w-5xl mx-auto px-6 py-4 flex items-center justify-between gap-4">
          <div class="flex items-center gap-3">
            ...
            <h1 class="text-xl font-bold text-slate-900">Chỉnh sửa Rubric</h1>
          </div>
          <div class="flex items-center gap-2">
            ...
            <button type="submit" form="rubricEditForm"
      ...
      <div class="max-w-5xl mx-auto px-6 py-6">
        ...
        @if (loading()) {
          <div class="bg-white rounded-xl border border-slate-200 p-24 text-center">
            <div class="w-10 h-10 border-2 border-slate-100 border-t-[#0056D2] rounded-full animate-spin mx-auto mb-4"></div>
            <p class="text-sm text-slate-400">Đang tải Rubric...</p>
          </div>
        } @else {
          <form id="rubricEditForm" [formGroup]="rubricForm" (ngSubmit)="saveRubric()"
                ...
                <div class="space-y-4">
                  <div>
                    ...
                    @if (rubricForm.get('name')?.invalid && rubricForm.get('name')?.touched) {
                      <p class="mt-1 text-xs text-rose-500">Tên rubric là bắt buộc.</p>
                    }
                  </div>
                  <div>
              ...
              <div class="bg-white border border-slate-200 rounded-xl p-6">
                ...
                @if (criteriaArray.length === 0) {
                  <div class="py-10 text-center border-2 border-dashed border-slate-200 rounded-lg">
                    ...
                    <p class="text-sm text-slate-400">Chưa có tiêu chí nào</p>
                  </div>
                } @else {
              ...
              <div class="sticky top-6 space-y-4">
                ...
                <div class="bg-white border border-slate-200 rounded-xl p-5">
                  ...
                  <div class="mt-4 pt-4 border-t border-slate-100 space-y-2">
                    ...
                    <p class="text-xs text-slate-400 flex items-start gap-2">
                      <span class="w-1 h-1 rounded-full bg-slate-300 mt-1.5 shrink-0"></span>
                      Thay đổi trọng số ảnh hưởng đến kết quả chấm bài cũ.
                    </p>
                  </div>
                </div>
      ...
      @if (success()) {
        <div class="fixed bottom-6 left-1/2 -translate-x-1/2 px-4 py-3 bg-emerald-600 text-white rounded-xl shadow-lg flex items-center gap-3 z-[200] text-sm font-medium">
          ...
          Rubric đã được cập nhật thành công!
        </div>
      }
    </div>
  `
...
export class RubricEditorComponent implements OnInit {
  ...
  saveRubric(): void {
    if (!this.rubricForm.valid || !this.isWeightValid()) {
      this.error.set('Vui lòng kiểm tra lại tính hợp lệ của Rubric (Tổng trọng số 100%)');
      return;
    }
