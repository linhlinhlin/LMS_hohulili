...
@Component({
    ...
    standalone: true,
    imports: [CommonModule, ReactiveFormsModule, MatDialogModule, MatButtonModule, MatInputModule, MatTabsModule, MatIconModule],
    template: `
        <div class="p-1">
                ...
                <mat-tab-group (selectedIndexChange)="onTabActiveChange($event)">
                    ...
                    <mat-tab label="Ghi danh Thủ công">
                        <form [formGroup]="form" class="space-y-4 pt-6 pb-2">
                            ...
                            <div *ngIf="manualError" class="text-red-600 text-sm bg-red-50 p-3 rounded border border-red-100 flex items-start gap-2">
                        ...
                        <div class="pt-6 space-y-4 pb-2">
                            ...
                            <div class="border-2 border-dashed rounded-xl p-8 text-center transition-all cursor-pointer group"
                                ...
                                <div class="flex flex-col items-center">
                                    <mat-icon class="text-5xl text-gray-300 mb-3 h-auto w-auto group-hover:text-blue-400" *ngIf="!selectedFile">cloud_upload</mat-icon>
                                    <mat-icon class="text-5xl text-green-500 mb-3 h-auto w-auto" *ngIf="selectedFile">description</mat-icon>
                                    
                                    <div class="text-sm font-medium text-gray-600" *ngIf="!selectedFile">
                                    ...
                                    <div class="text-sm text-green-700 font-bold flex items-center gap-2" *ngIf="selectedFile">
                                        {{ selectedFile.name }}
                            ...
                            <!-- Excel Error -->
                            <div *ngIf="excelError" class="text-red-600 text-sm bg-red-50 p-3 rounded border border-red-100 flex items-center gap-2">
                                <mat-icon class="text-sm scale-75">warning</mat-icon>
                                <span>{{ excelError }}</span>
                            </div>

                            <!-- Import Results -->
                            <div *ngIf="importSummary" class="space-y-4 pt-2 border-t mt-4 animate-fadeIn">
                                <div class="flex items-center justify-between bg-gray-50 p-2 rounded">
                                    <div class="flex gap-4 text-xs font-bold">
                                        ...
                                        <span class="text-red-600 flex items-center gap-1" *ngIf="importSummary.failures.length > 0">
                                            <mat-icon class="text-xs scale-75">error</mat-icon> Lỗi: {{ importSummary.failures.length }}
                                        </span>
                                    </div>
                                </div>
                                
                                <div *ngIf="importSummary.failures.length > 0" class="border rounded-lg overflow-hidden divide-y divide-gray-100">
                                    <div class="bg-gray-50 p-2 text-[10px] font-bold text-gray-500 uppercase tracking-wider">Danh sách dòng lỗi</div>
                                    <div class="max-h-32 overflow-y-auto">
                                        <div *ngFor="let f of importSummary.failures" class="p-2.5 flex items-center gap-3 text-xs hover:bg-gray-50 transition-colors">
