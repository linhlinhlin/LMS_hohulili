...
@Component({
  ...
  template: `
    <!-- Backdrop -->
    @if (isOpen()) {
      <div class="fixed inset-0 bg-black/40 backdrop-blur-[2px] z-[1000] transition-opacity"
           (click)="close()"
           [@backdropAnimation]>
      </div>

      <!-- Drawer Panel -->
      <div class="fixed top-0 right-0 h-full bg-white shadow-2xl z-[1001] flex flex-col overflow-hidden"
           [style.width]="width()"
           [@drawerAnimation]>
        ...
        <!-- Footer (Optional) -->
        @if (showFooter()) {
          <div class="p-4 border-t bg-gray-50/50 flex justify-end gap-3">
