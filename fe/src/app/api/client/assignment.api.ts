...
export class AssignmentApi {
  ...
  getAssignmentsByCourse(courseId: string, params?: { page?: number; limit?: number }) {
    return this.api.getWithResponse<any>(`/api/v3/courses/${courseId}/assignments`, { params }).pipe(
  ...
  createAssignment(courseId: string, payload: CreateAssignmentRequest) {
    return this.api.postWithResponse<AssignmentDetail>(`/api/v3/courses/${courseId}/assignments`, payload);
  }

  // Get assignment detail
  getAssignmentById(assignmentId: string) {
    return this.api.getWithResponse<AssignmentDetail>(`/api/v3/assignments/${assignmentId}`);
  }

  // Update assignment
  updateAssignment(assignmentId: string, payload: UpdateAssignmentRequest) {
    return this.api.putWithResponse<AssignmentDetail>(`/api/v3/assignments/${assignmentId}`, payload);
  }

  // Delete assignment
  deleteAssignment(assignmentId: string) {
    return this.api.deleteWithResponse<string>(`/api/v3/assignments/${assignmentId}`);
  }
  ...
  getTeacherAssignments(params?: { page?: number; limit?: number; courseId?: string; status?: string }) {
    ...
    return this.api.getWithResponse<AssignmentSummary[]>(`/api/v3/assignments/teacher-summary`, { params });
  }
}
