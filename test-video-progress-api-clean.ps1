...
try {
    ...
    if ($canProceedResponse.success) {
        ...
        if ($canProceed) {
            Write-Host "     UNEXPECTED: Can proceed at 40%!" -ForegroundColor Red
        } else {
...
try {
    ...
    if ($trackResponse80.success) {
        ...
        if ($completed) {
            ...
            Write-Host "     UNEXPECTED: Not completed at 80%!" -ForegroundColor Red
        }
    }
} catch {
    Write-Host "    Track error: $($_.Exception.Message)" -ForegroundColor Red
...
try {
    ...
    if ($canProceedResponse80.success) {
        ...
        if ($canProceed) {
            ...
            Write-Host "     UNEXPECTED: Still locked at 80%!" -ForegroundColor Red
        }
        Write-Host "   Progress: $currentProgress%" -ForegroundColor Gray
        Write-Host "   Message: $message" -ForegroundColor Gray
    }
