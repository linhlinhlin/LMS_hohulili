...
public class UserControllerV3 {
    ...
    public ResponseEntity<ApiResponse<Page<UserResponse>>> searchUsers(
        ...
        if (role != null && !role.isBlank() && q != null && !q.isBlank()) {
            users = userRepository.findByRoleAndEmailContainingIgnoreCaseOrFullNameContainingIgnoreCase(
                    role.toUpperCase(), q, q, pageable);
        } else if (role != null && !role.isBlank()) {
            users = userRepository.findByRole(role.toUpperCase(), pageable);
        } else if (q != null && !q.isBlank()) {
            users = userRepository.findByEmailContainingIgnoreCaseOrFullNameContainingIgnoreCase(q, q, pageable);
        } else {
            users = userRepository.findAll(pageable);
        }
