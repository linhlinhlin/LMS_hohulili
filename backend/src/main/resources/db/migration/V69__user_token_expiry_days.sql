...
ALTER TABLE organizations DROP CONSTRAINT IF EXISTS chk_token_expiry;
ALTER TABLE organizations ADD CONSTRAINT chk_token_expiry
    CHECK (token_expiry_days >= 7 AND token_expiry_days <= 1095);
