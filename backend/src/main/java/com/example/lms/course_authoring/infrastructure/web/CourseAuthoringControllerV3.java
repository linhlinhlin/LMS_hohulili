...
public class CourseAuthoringControllerV3 {
    ...
    public ResponseEntity<ApiResponse<com.example.lms.shared.domain.model.ContentBlock>> addSection(
        ...
        try {
            // Handle file if present (TODO: Implement proper storage service)
            if (file != null && !file.isEmpty()) {
                System.out.println("Received file: " + file.getOriginalFilename());
            }

            String type = (String) payload.getOrDefault("type", "TEXT");
            System.out.println("Processing addSection for lesson: " + lessonId + ", type: " + type);
            ...
            throw e; // Re-throw to let global handler return 500, but we captured the log
        }
    }
    ...
    @PutMapping("/{courseId}")
    @PreAuthorize("hasAnyRole('ADMIN', 'TEACHER')")
    public ResponseEntity<ApiResponse<ChapterResponse>> updateCourse(
            @PathVariable UUID courseId,
        ...
        // Map CourseResponse to ChapterResponse? No, generic object or specific response.
        // The return type should be ApiResponse<CourseResponse>
        return ResponseEntity.ok(ApiResponse.success(null, "Course updated successfully")); 
    }
