...
class RefreshTokenUseCaseV2Test {
    ...
    void shouldRejectDisabledUser() {
        ...
        verify(tokenService, never()).generateRefreshToken(any(), anyString(), anyString());
    }
    ...
    void shouldRefreshTokenForEnabledUser() {
        ...
        when(tokenService.generateAccessToken(eq(userId.value()), eq(TEST_EMAIL), eq("STUDENT")))
        ...
        verify(tokenService).generateAccessToken(userId.value(), TEST_EMAIL, "STUDENT");
        verify(tokenService).generateRefreshToken(userId.value(), TEST_EMAIL, "STUDENT");
    }
}
